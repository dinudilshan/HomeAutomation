...
function addDevice($DeviceName, $DeviceID, $UserID ,$conn) {
    ...
    try{
        ...
        logme($result['uid'],time(),"PDOException","SELECT * FROM users WHERE uid=:uid","Error", $exception);
    }


    if ($stmt->rowCount() == 1) {
        try{
            $stmt = $conn->prepare("INSERT INTO devices (DeviceName, DeviceID, UserID, time)
            VALUES (:DeviceName, :DeviceID, :UserID, :time)");
            ...
            logme($username,time(),"INSERT INTO devices (DeviceName, DeviceID, UserID, time)
            VALUES (:DeviceName, :DeviceID, :UserID, :time)","Error", $exception, "n/a");
            return "1";
        }
        
    }
    else {
        logme($result['uid'],time(),"add_device","N/A","Error", "UserID not found", "user found");
        return "2"; //uid not found
    }
